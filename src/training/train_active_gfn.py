...
def run_active_learning(
    ...
    gfn_cfg = active_cfg.get("gfn", {})
    gfn_n_steps    = int(gfn_cfg.get("n_steps", 500))    
    ...
    for round_idx in range(max_rounds):
        ...
        def reward_fn(terminal_states: np.ndarray) -> np.ndarray:
            assert surrogate is not None, "surogate doit être initialisé avant reward_fn"
            mean, std = surrogate.predict(terminal_states, return_std=True)
            acquisition = ucb_scores(mean=mean, std=std, beta=beta)
            return np.exp(acquisition)  
        ...
        candidate_states = sample_candidates(
            gfn=gfn,
            env=env,
